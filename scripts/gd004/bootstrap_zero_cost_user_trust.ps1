...
# PowerShell 5.1 does not turn native nonzero exit codes into terminating errors.
function Assert-NativeSuccess([string]$Operation) {
  if ($LASTEXITCODE -ne 0) {
    throw "$Operation failed (exit code $LASTEXITCODE). Bootstrap stopped."
  }
}
...
Assert-NativeSuccess 'git source lookup'
$RemoteSha = (git ls-remote origin refs/heads/main).Split("`t")[0].Trim()
Assert-NativeSuccess 'remote main lookup'
...
Assert-NativeSuccess 'workflow run lookup'
...
Assert-NativeSuccess 'TEST dispatch'
Start-Sleep -Seconds 3
...
Assert-NativeSuccess 'live TEST workflow'
